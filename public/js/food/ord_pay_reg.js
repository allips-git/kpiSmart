 ...
 * @author 김민주, @version 1.0, @last date 2022/08/09
 ...
 $(function () {
    ...
    $("#btn_reg").off().click(function () { 
        ...
        {
            $("#p").val("in");
            ord_pay_validation($("#frm_reg").serializeObject()); // form 데이터 유효성 검사
        }
    });
 ...
 */
 function ord_pay_validation(obj) 
 {
    $.ajax({
        url: '/acc/ord_pay/v',
        type: 'POST',
...
/**
 * @description 수금/금액차감 등록
 * @return result code, comment
 */
function ord_pay_register(obj) 
{
    $.ajax({
