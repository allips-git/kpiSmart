 ...
 * @author 김민주, @version 1.1, @last date 2022/07/21 - 재고DB 리뉴얼
 ...
 $(function () {
    ...
    // 원자재 재고 조회
    get_item_st_list($("#frm_search").serializeObject(), 'Y');
...
/**
 * @description 원자재 재고 리스트 - ajax 페이지네이션
 * @document URL: https://pagination.js.org/docs/index.html
*/
function get_item_st_list(obj, mode='') 
{
    const container = $('#pagination');
    $.ajax({

        url: '/stock/item_st/list',
        type: 'POST',
        data: obj,
        dataType: "json",
        async: false,
                ...
                callback: function (result, pagination) {
                    ...
                    if (count > 0) 
                    {
                        $.each (result, function (i, list) 
                        {
                            ...
                            str += '<td class="T-right">'+ commas(list.in_qty) +'</td>';
                            str += '<td class="T-right">'+ commas(list.pr_out_qty) +'</td>';
                            str += '<td class="T-right">'+ commas(list.re_out_qty) +'</td>';
