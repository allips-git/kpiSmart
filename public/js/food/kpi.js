...
function createChart(dataList) {
    ...
    // 현재 달의 평균 수량 계산
    const currentMonth = new Date().toISOString().substring(0, 7); // 현재 'YYYY-MM' 형식
    const currentMonthData = aggregatedData[currentMonth] || 0;

    console.log("현재 달 평균 수량: ", currentMonthData);

    $('#aver').text("현재 달 평균 수량: " +  currentMonthData + " 건");
