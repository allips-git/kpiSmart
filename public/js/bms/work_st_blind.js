/*================================================================================
 * @name: 황호진 - work_st_blind.js	작업현황 블라인드 화면
 * @version: 1.0.0, @date: 2022-04-15
 ================================================================================*/
$(function () {
	//화면이 맨처음 로드 될때!
	//================================================================================
...
function get_list(data) {
	...
	var url = '/work/work_st_blind/get_list';
	var type = 'GET';
...
function get_easy_search_list(data) {
	var container = $('#pagination');	//pagination
	var url = '/work/work_st_blind/get_easy_search_list';
	var type = 'GET';
	fnc_ajax(url , type , data)
		.done(function (res) {
			//작업완료 치기 위한 기준데이터
...
function draw_list(draw_data , ref_list) {
	...
	if(draw_len > 0){
		$.each(draw_data , function (i , list) {
			...
			str += '<td>'+ item_gb.item_gb +'</td>';
			str += '<td class="w7">'+ ord_spec.ord_width +' X '+ ord_spec.ord_height +'</td>';
			str += '<td class="w7 count">';
			str += '<span>좌 : '+ ord_qty.left_qty +'</span><span>우 : '+ ord_qty.right_qty +'</span>';
			str += '</td>';
			str += '<td class="w5">'+ ord_spec.ord_hebe +'</td>';
...
function work_comp(arr) {
	var url = '/work/work_st_blind/work_comp';
	var type = 'POST';
	var data = {
		works : arr
	};
