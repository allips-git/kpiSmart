/*================================================================================
 * @description 공장 시스템 센터 주문 내역 상세보기 화면 JS
 * @author 황호진, @version 1.0, @last date 2022/05/27
 ================================================================================*/
$(function () {
	//화면이 맨처음 로드 될때!
	//================================================================================
		...
		if(chk_arr.length > 0){
			...
			con.then((result) => {
				if(result.isConfirmed){
					var url = '/cen/ord_com_c/appr_can';
					var type = 'POST';
					var data = {
						ord_no	: $('#ord_no').val(),
						seq_arr	: chk_arr
...
function get_ord_list(data) {
	var url = '/cen/ord_com_c/get_ord_list';
	var type = 'get';
	fnc_ajax(url , type , data)
		.done(function (res) {
			var str = '';
...
function bunhal_detail(id , ord_no , ord_seq) {
	var url = '/cen/ord_com_c/bunhal_detail';
	var type = 'GET';
	var data = {
		'ord_no'	: ord_no,
		'ord_seq'	: ord_seq,
