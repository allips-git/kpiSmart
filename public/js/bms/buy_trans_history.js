/*================================================================================
 * @name: 황호진 - buy_trans_history.js	매입처별 거래내역 js
 * @version: 1.0.0, @date: 2022-08-23
 ================================================================================*/
let start_num = 0;
let stat = true;
const max_num = 50;
...
//화면이 맨처음 로드 될때!
//================================================================================
get_bth_list(bth_search_organize() , 'load');
//================================================================================
...
$('#content').on("scroll", function(){
	...
	if (scroll_top + inner_height >= scroll_height) // 스크롤 끝 부분쯤 도착했을 시
	{
		if(stat)
		{
			get_bth_list(bth_search_organize() , 'scroll');
		}
	}
});
...
$('#st_dt , #ed_dt').on('change' , function () {
	start_num = 0;
	stat = false;
	get_bth_list(bth_search_organize() , 'change');
});
//================================================================================
...
/**
 * @description get_bth_list
 * @author 황호진  @version 1.0, @last update 2022-08-24
 */
function get_bth_list(data , event_type) {
	let url = '/ledg/buy_trans_history/get_bth_list';
	let type = 'GET';
		...
		.done(function (res) {
			...
			if(len > 0)
			{
				$.each(res.data , function (i , list) {
					str += '<tr onclick=location.href="/ledg/buy_trans_history/v?cc='+ list.cust_cd +'">';
					...
					str += '<td class="w10 T-right">'+ commas(Number(list.buy_amt)) +'</td>';
					str += '<td class="w10 T-right">'+ commas(Number(list.re_amt)) +'</td>';
					str += '<td class="w10 T-right">'+ commas(Number(list.wd_amt)) +'</td>';
