...
function get_detail(cust_cd) {
	...
	fnc_ajax(url , type , data)
		.done(function (res) {
			...
			//가용여부 설정
			if(res.useyn === 'Y') $("#useyn1").prop("checked",true);
			else if(res.useyn === 'N') $("#useyn2").prop("checked",true);
			...
			//결제조건
			$("#pay_gb").val(res.pay_gb);
			//거래처 구분
			$("#cust_gb").val(res.cust_gb);
			//거래처
			$("#cust_grade").val(res.cust_grade);
			//배송구분
			$("#dlv_gb").val(res.dlv_gb);
			//거래처명
			$("#cust_nm").val(res.cust_nm);
			//사업자명
			$("#biz_nm").val(res.biz_nm);
			//사업자구분
			$("#biz_gb").val(res.biz_gb);
			//사업자등록번호
			$("#biz_num").val(res.biz_num);
			//업태
			$("#biz_class").val(res.biz_class);
			//종목
			...
			$("#email").val(res.email);
			//사업장 우편번호
			$("#biz_zip").val(res.biz_zip);
			//사업장 소재지
			$("#address").val(res.address);
			//사업장 상세소재지지
			$("#addr_detail").val(res.addr_detail);
			//대표자
			$("#ceo_nm").val(res.ceo_nm);
			//대표자연락처
			$("#ceo_tel").val(res.ceo_tel);
			//담당자
			$("#person").val(res.person);
			//담당자연락처
			$("#person_tel").val(res.person_tel);
			//예금주 입력
			...
			//영업 담당자
			$("#sales_person").val(res.sales_person);
			//부가세
			$("#vat").val(res.vat);
			//미수금한도
			$("#limit_amt").val(Number(res.limit_amt).toLocaleString('ko-KR'));
			//비고
			$("#memo").val(res.memo);

			//배송지관리
			$("#ba_nm_btn").css('display' , 'inline');
			$("#ba").css('display' , 'none');
...
function useyn_change(cust_cd) {
	var url = '/biz/buying/useyn_change';
	var type = 'POST';
	var data = {
		'cust_cd' : cust_cd
	};
...
function input_check() {
	...
	//배송지명과 배송주소 검사
	if(p === 'in'){
		var ba_list = ['ba_nm' , 'ba_zip' , 'ba_addr'];
		var ba_flag = true;
