/*================================================================================
 * @description 공장 시스템 센터 주문 승인 상세화면
 * @author 황호진, @version 1.0, @last date 2022/05/23
 ================================================================================*/
$(function () {
	//화면이 맨처음 로드 될때!
	//================================================================================
	get_ord_list_c({ord_no : $('#ord_no').val()});
	//================================================================================
	...
	* @author 황호진  @version 1.0, @last update 2022/05/23
	*/
	...
	* @author 황호진  @version 1.0, @last update 2022/05/24
	*/
	$('#appr_comp_c_btn').on('click' , function () {
	...
	*/
	$('#ord_can_c_btn').on('click' , function () {
	...
	$(document).on('click' , '.bunhal' , function () {
		...
		bunhal_detail_c(id , ord_no , ord_seq);
	});
 ...
 * @author 황호진  @version 1.0, @last update 2022/05/23
 */
function get_ord_list_c(data) {
	var url = '/cen/ord_wait/get_ord_list_c';
	var type = 'get';
	fnc_ajax(url , type , data)
		.done(function (res) {
			...
			//토탈 합계 금액
			$('#total_amt_c').html(commas(total_amt)+'원');
 ...
 * @author 황호진  @version 1.0, @last update 2022/05/24
 */
function appr_comp_c() {
	var url = '/cen/ord_wait/appr_comp_c';
	var type = 'POST';
	var data = {
		ord_no	: $('#ord_no').val(),
		dlv_dt	: $('#dlv_dt').val()
 ...
 */
function appr_rej_c() {
	var url = '/cen/ord_wait/appr_rej_c';
	var type = 'POST';
	var data = {
		ord_no	: $('#ord_no').val(),
	};
 ...
 * @author 황호진  @version 1.0, @last update 2022/05/25
 */
function bunhal_detail_c(id , ord_no , ord_seq) {
	var url = '/cen/ord_wait/bunhal_detail_c';
	var type = 'GET';
	var data = {
		'ord_no'	: ord_no,
		'ord_seq'	: ord_seq,
	...
	fnc_ajax(url , type , data)
		.done(function (res) {
			...
			str += '<span id="bunhal_close">&times;</span>';
			str += '<p>분할정보</p>';
			str += '<ul>';
			$.each(res.data, function (i, list) {
				str += '<li>';
			...
			var target_info = document.getElementById('bunhal_detail_c');
			...
			$('#bunhal_detail_c').css({"top": y+"px", "left": x+"px"});
		}).fail(fnc_ajax_fail);
}
 ...
 */
function ord_can_c() {
	var url = '/cen/ord_wait/ord_can_c';
	var type = 'POST';
	var data = {
		ord_no	: $('#ord_no').val(),
	};
