/*================================================================================
 * @description 공장 시스템 센터 주문 승인(대기) 메인페이지 관리 JS
 * @author 김민주, @version 1.0, @last date 2021/12/20
 ================================================================================*/

  $(function () {
    
    // 공장별 주문 승인(대기) 내역 조회
    get_ord_list({'local_cd':$("#local_cd").val()}, "Y"); 
                ...
                callback: function (result, pagination) { 
                    ...
                    if(count > 0) {
                        ...
                        $.each(result, function (i, list) {
                            ...
                            str += '<tr onclick=location.href="/cen/ord_wait/in?no='+list.ord_no+'";>';
                            str += '<td class="w8">' + list.rownum + '</td>';
                            str += '<td class="w8">' + list.ord_dt + '</td>';
                            str += '<td class="w10 tb_click">' + list.ord_no + '</td>';
                            str += '<td class="T-left td_left tb_click">' + list.cust_nm + '</td>';
                            str += '<td class="T-left td_left tb_click">' + list.cen_nm + '</td>';
                            str += '<td class="w10">' + commas(parseFloat(list.total_amt))+ '&nbsp원' + '</td>';
                            str += '<td class="w10">' + list.ord_seq; + '</td>';
                            str += '<td class="w10">' + commas(list.total_qty) + '&nbsp;개' +'</td>';
                            str += '</tr>';
