...
/**
 * @description 원자재 관리 컨트롤러
 * @author 안성준, @version 2.0, @last date 2022/06/27
 */
class Buy_item_m extends CI_Model {
    ...
    public function get_list($data)
    {
        ...
        $sql = 'SELECT  @rownum := @rownum+1 AS rownum, sub.*
                FROM (
                    ...
                    WHERE i.local_cd = ? AND i.item_gb IN ("004", "005", "006") AND i.delyn = "N"
    ...
    /**
     * @description 원자재 관리 상세 조회
     * @param 공장코드, 제품코드
     * @return item detail
     */
    public function get_detail($data)
    {
        $sql = 'SELECT i.ikey, i.local_cd, i.pd_cd, i.item_cd, i.item_gb, i.item_lv, i.item_nm, i.size, i.unit, cc.code_nm AS unit_nm
                , i.min_size, i.unit_amt, i.sale_amt, i.wh_uc, i.safe_qty, i.memo, i.useyn, i.delyn, i.buy_cd
                , JSON_UNQUOTE(JSON_EXTRACT(i.spec, "$.maker")) AS maker
                , JSON_UNQUOTE(JSON_EXTRACT(i.spec, "$.origin")) AS origin
                FROM item_list AS i
    ...
    /**
     * @description 원자재 등록
     * @hint 원자재 등록시 매입거래처, 제품 분류, 기본 창고 지정 있을경우 창고 사용(sysyn) 업데이트
     */
    public function insert($reg, $link)
    {
        ...
        if ($reg['buy_cd'] != '') // 매입거래처
        {
            $this->db->update('biz_list', $link, array('cust_cd'=>$reg['buy_cd']));
        }
    ...
    /**
     * @description 원자재 수정
     * @hint 원자재 수정시 매입거래처, 제품 분류, 기본 창고 지정 있을경우 창고 사용(sysyn) 업데이트
     */
    public function update($mod, $data, $link)
    {
        ...
        if ($mod['buy_cd'] != '') // 매입거래처
        {
            $this->db->update('biz_list', $link, array('cust_cd'=>$mod['buy_cd']));
        }
