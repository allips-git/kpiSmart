...
/**
 * @description 매입/지급 관리 모델
 * @author 김민주, @version 1.0, @last date 2022/08/10
 */
class Buy_pay_m extends CI_Model {
    ...
    public function get_list($data)
    {
        ...
        $sql = 'SELECT  @rownum := @rownum+1 AS rownum, sub.*
                FROM (
                    SELECT a.ikey, a.local_cd, a.cust_cd, a.acc_dt, a.detail, ac.code_nm AS detail_nm, a.acc_type
                    ...
                    , IFNULL((SELECT SUM(amt+tax) FROM buy_acc_list 
                              WHERE cust_cd = a.cust_cd AND acc_no <= a.acc_no AND `work` = "IN"
                              AND useyn = "Y" AND delyn = "N"), 0) 
                    - IFNULL((SELECT SUM(amt+tax) FROM buy_acc_list 
                              WHERE cust_cd = a.cust_cd AND acc_no <= a.acc_no AND `work` = "OUT"
                              AND useyn = "Y" AND delyn = "N"), 0) AS total_amt
                    FROM buy_acc_list AS a
                    INNER JOIN (SELECT @rownum := 0) r
                    INNER JOIN z_plan.common_code AS ac ON (ac.code_gb ="AC" AND ac.code_main = "130" AND a.detail = ac.code_sub)   # 거래 상세
                    LEFT JOIN buy_master AS m ON (a.local_cd = m.local_cd AND a.ord_no = m.ord_no)
