...
/**
 * @description 매출/수금 관리 컨트롤러
 * @author 김민주, @version 1.0, @last date 2022/08/09
 */
class Ord_pay extends CI_Controller {

    protected $table  = 'ord_acc_list';
	...
	public function index()
	{
        ...
        $data['title'] = '매출/수금 관리';
        $data['site_url'] = '/acc/ord_pay';
        $this->load->view('include/head');
        $this->load->view('include/bms_head');
        $this->load->view('include/bms_side');
        $this->load->view('acc/ord_pay', $data);
        $this->load->view('include/acc/ord_pay_reg', $data);
        // $this->load->view('include/acc/ord_pay_mod', $data);
        $this->load->view('include/bms_tail');
        $this->load->view('include/tail');
	}

    /**
     * @description 매출/수금 전체 조회/검색 조회
     * @param 공장코드, 거래처코드, 검색 날짜
     * @return ord acc list [json] 
     */
    public function list()
    {
        ...
        $data['list'] = $this->Ord_pay_m->get_list($data['var']);
        exit(json_encode(['result'=>$data])); // return result list
    }

    /**
     * @description 매출/수금 등록 - insert
