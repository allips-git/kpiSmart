...
/**
 * @description 제품 분류 관리(Item gubun) 컨트롤러
 * @author 김민주, @version 1.0, @last date 2022/06/03
 */
class Item_gb extends CI_Controller {
	...
	{ 
        ...
        $this->load->model('base/Item_gb_m');
	}

    /**
     * @description 제품분류 리스트 전체 조회/검색 조회
    ...
    public function list()
    {
        ...
        $data['list'] = $this->Item_gb_m->get_list($data['var']);
        exit(json_encode(['result'=>$data])); // return result list
    }

    /**
     * @description 제품분류 상세페이지
     * @param ikey
     * @return detail [json] 
     */
    ...
    {   
        ...
        $data['detail'] = $this->Item_gb_m->get_detail($data['var']);
        exit(json_encode(['result'=>$data])); // return result list
    }

    /**
     * @description 제품분류 등록 - insert
     * @return result code [json] 
     */
    public function i()
    {
        // 공장별 제품군, 소속제품군 기본값 설정
        $local_cd = $this->session->userdata['local_cd'];
    ...
    public function v() 
    {
        if (isset($_POST['p'])) 
        {
            ...
            $config = array(
                ...
                array('field'=>'key_name',     'label'=>'제품 분류는',          'rules'=>'trim|required')
            );
